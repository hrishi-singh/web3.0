...
const customStyles = {
  ...
  option: (provided, state) => ({
    ...provided,
    backgroundColor: state.isSelected ? '#555' : state.isFocused ? '#666' : '#333', // Customize option background color
...
const Exchange = () => {
  ...
  const handleConvert = async() => {
    if (selectedOption) {
      const conversion =parseFloat(selectedOption.value) * parseFloat(numberInput);
  ...
  const coins = async () =>{
    ...
    const opt= async()=>{
      ...
      const API_URL =
      `https://min-api.cryptocompare.com/data/pricemulti?fsyms=ETH&tsyms=${op}&api_key=6a4234186f13b2ca0bef7921b1ff54662c76f2bf64a814af19f9267bbdd48662`;
          ...
          <div className="flex">
            ...
            <div className="flex flex-1 justify-start flex-col mf:mr-10">
              ...
              handleChange={handleInputChange} />
                ...
                <Select styles={customStyles} className="my-2 w-full rounded-sm p-2 outline-none bg-transparent border-none white-glassmorphism" options={options} 
